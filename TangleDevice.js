...
export class TangleDevice {
      ...
      if (!this.socket) {
         ...
         this.socket.on("deliver", async (reqId, payload) => {
            ...
            this.interface
               ...
               .catch((error) =>
                  this.socket.emit("response_error", reqId, error)
               );
         });
   ...
   adopt(
      ...
      return this.interface
         ...
         .then(() => {
            ...
            const device_name_bytes = stringToBytes(
               newDeviceName.slice(0, 11),
               16
            );
            ...
            return this.interface
               ...
               .then((response) => {
                  ...
                  logging.debug(
                     `error_code=${error_code}, device_mac=${device_mac}`
                  );
   ...
   connect(
      ...
      return (
         ...
         .catch((error) => {
            ...
            logging.error(error);
            // if (error === "UserCanceledSelection") throw{name:"Connection cancelled"}
            // else
            if (error === "UserCanceledSelection" || error === "BluefyError") {
               throw {
                  name: t("Připojení selhalo"),
                  message: t(
   ...
   updateNetworkFirmware(firmware) {
      ...
      return (
         window
            //@ts-ignore
            .confirm(
               t("Nastaví rychlejší přenos dat, který ale nemá takový dosah."),
               t("Jsou zařízení blízko sebe?"),
               { confirm: "Ano", secondary: "Ne" }
            )
            //@ts-ignore
            .then((result) => {
               if (result) {
                  ...
                  try {
...

                     {
                        ...
                        while (written < firmware.length) {
                           ...
                           const percentage =
                              Math.floor((written * 10000) / firmware.length) /
                              100;
   ...
   getTnglFingerprint() {
      ...
      return this.interface.request(bytes, true).then((response) => {
         ...
         if (
            reader.readFlag() !== DEVICE_FLAGS.FLAG_TNGL_FINGERPRINT_RESPONSE
         ) {
            throw "InvalidResponseFlag";
         }
   ...
   getConnectedPeersInfo() {
      ...
      return this.interface.request(bytes, true).then((response) => {
         ...
         if (
            reader.readFlag() !==
            DEVICE_FLAGS.FLAG_CONNECTED_PEERS_INFO_RESPONSE
         ) {
            throw "InvalidResponseFlag";
         }
